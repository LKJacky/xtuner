...
try:
    from flash_attn.flash_attn_interface import flash_attn_gpu, round_multiple, maybe_contiguous
except ImportError:
    pass
...
def flash_attn_varlen_func_v2(
    ...
    """dropout_p should be set to 0.0 during evaluation Supports multi-query
    and grouped-query attention (MQA/GQA) by passing in K, V with fewer heads
    than Q. Note that the number of heads in Q must be divisible by the number
    of heads in KV. For example, if Q has 6 heads and K, V have 2 heads, head
    0, 1, 2 of Q will attention to head 0 of K, V, and head 3, 4, 5 of Q will
    attention to head 1 of K, V.
